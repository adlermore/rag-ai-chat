"""
BM25 (Okapi) — pure-python, идентичен реализации Фазы 0 (тем самым сохраняется
валидированное поведение гибрида). Токенизация — армянская (tokenizer_hy).

Индекс держится в памяти ingest-сервиса; строится при ингестии/переиндексации.
"""
...
class BM25:
    ...
    def search(self, query: str, top_k: int | None = None) -> list[str]:
        """doc_id по убыванию BM25-score (только с положительным score)."""
